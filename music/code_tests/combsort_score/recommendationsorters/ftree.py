...
class FTree:
	...
	def build(self):
		...
		for item in self.items:
			self._buildtime_add_item(item)
	
	def _buildtime_add_item(self,item):
		self.root.count+= 1
	...
	def _post_build_add_item(self,item):
		#Add it to the tree
		self._build_time_add_item(self,item)
		#Manually update self.frequency
		for keytuple in self.field_combo_cardinality:	#Because i get the field keys here
			ftuple = (keytuple[0],)
			for i in range(1,len(keytuple)):
				if keytuple[i] is None:
					ftuple+= (None,)
				else:
					ftuple+= ( getattr(item,keytuple[i]), )
			
			if self.frequency[ftuple] is None:
				self.frequency[ftuple] = 1
				self.field_combo_cardinality[keytuple]+=1
			else:
				self.frequency[ftuple] += 1
			#Update stats
			if self.frequency[ftuple] > self.max_freq[keytuple]:	
				self.max_freq[keytuple] = self.frequency[ftuple]

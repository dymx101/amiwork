...
for sf in scoring_functions:
	...
	for song in combosorter.sorted:
		result_file.write("%40s %40s %20s %20s %20s %20s\n" % (song.title, song.artist, song.genre, song.mood, song.tempo, combosorter.score[song]) )
	result_file.close()
...
for sf in scoring_functions:
	...
	for song in plsorter.sorted:
		result_file.write("%40s %40s %20s %20s %20s %20s\n" % (song.title, song.artist, song.genre, song.mood, song.tempo, plsorter.score[song]) )
	result_file.close()
#'''
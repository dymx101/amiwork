...
class SimpleFPMiner:
	...
	def gen_conditional_tree(self,suffixid):
		...
		self.conditional_tree[suffixid] = SimpleFPTree()
		...
		for (itemid_list,count) in self.conditional_base[suffixid]:
			...
			i+=1
		#print fpt.transactions
		fpt.compute_L()
		
		fpt.construct_tree()
		#print "PRE PRUNE:";		fpt.print_tree()
		fpt.set_min_support(self.tree.min_support)
		fpt.prune_tree()
		#print "POST PRUNE:";fpt.print_tree()
	...
	'''
	def mine(self):	
		""" Generates frequency patterns for self.tree """
		print "---ENTER---"
		...
		for suffix in self.conditional_tree:
			...
			#Base case: The tree is a single node => It has no children
			' '' #Done when appending the suffixes themselves
			if not subtree.root.children:
				print "No children",suffix
				#self.freq_patterns.append( ( [suffix], subtree.root.count) )
				continue
			' ''
			...
			#Replace this with a list concat operation
			for fpattern in result:
				self.freq_patterns.append(fpattern)
	...
	def mine(self):	#Simpler version
		...
		for suffix in self.conditional_tree:
			...
			for fpattern in result:
				...
				patt.append(suffix)
				self.freq_patterns.append( (patt,c) )
			print "Patterns of subtree", self.freq_patterns
			print "------------"
